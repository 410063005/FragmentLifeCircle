...
class MyBottomSheetDialog : BottomSheetDialogFragment() {
    ...
    companion object {
        fun newInstance(layoutId: Int? = null): MyBottomSheetDialog {
            val d = MyBottomSheetDialog()
            if (layoutId != null) {
                val a = Bundle()
                a.putInt("layout-id", layoutId)
                d.arguments = a
            }
            return d
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val layoutResource = arguments?.getInt("layout-id") ?: R.layout.fragment_my_bottom_dialog_fixed
